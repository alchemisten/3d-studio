...
import { GLTF2 } from '@alchemisten/gltf-extension';
...
describe('GLTF Extension Validation', () => {
  ...
  const gltf: GLTF2.GLTF = JSON.parse(raw.toString());
  ...
  it(`Validation by normal validator`, () => {
    ...
    validateBytes(new Uint8Array(raw), validationOptions).then((_report) => {
      validReport(_report);
    });
  });
  ...
  it(`${ALCM_INTERACTIVITY_EXT_NAME} extension defined`, () => {
    equal(
      gltf.extensions.hasOwnProperty(ALCM_INTERACTIVITY_EXT_NAME),
      true,
      `No ${ALCM_INTERACTIVITY_EXT_NAME} extension found`
    );
  });
});
